...
#include <scene_loading/tinyxml2.h>
...
namespace TPM_NAMESPACE {
  ...
  Transform Transform::fromIdentity() {
    return Transform(Transform::Array{
        {Number(1), Number(0), Number(0), Number(0), Number(0), Number(1), Number(0), Number(0),
         Number(0), Number(0), Number(1), Number(0), Number(0), Number(0), Number(0), Number(1)}});
  }

  Transform Transform::fromTranslation(const Vector& delta) {
    return Transform(Transform::Array{
        {Number(1), Number(0), Number(0), delta.x, Number(0), Number(1), Number(0), delta.y,
         Number(0), Number(0), Number(1), delta.z, Number(0), Number(0), Number(0), Number(1)}});
  }
  Transform Transform::fromScale(const Vector& scale) {
    return Transform(Transform::Array{{scale.x, Number(0), Number(0), Number(0), Number(0), scale.y,
                                       Number(0), Number(0), Number(0), Number(0), scale.z,
                                       Number(0), Number(0), Number(0), Number(0), Number(1)}});
  }
  ...
  Transform Transform::fromRotation(const Vector& axis, Number angle_degree) {
    ...
    return Transform(Transform::Array{
        {ca + aa.x * aa.x * nca, aa.x * aa.y * nca - aa.z * sa, aa.x * aa.z * nca + aa.y * sa,
         Number(0), aa.y * aa.x * nca + aa.z * sa, ca + aa.y * aa.y * nca,
         aa.y * aa.z * nca - aa.x * sa, Number(0), aa.z * aa.x * nca - aa.y * sa,
         aa.z * aa.y * nca + aa.x * sa, ca + aa.z * aa.z * nca, Number(0), Number(0), Number(0),
         Number(0), Number(1)}});
  }

  Transform Transform::fromLookAt(const Vector& origin, const Vector& target, const Vector& up) {
    const Vector fwd
        = normalize(Vector(target.x - origin.x, target.y - origin.y, target.z - origin.z));
    const Vector left = normalize(cross(up, fwd));
    const Vector alt_up = normalize(cross(fwd, left));

    return Transform(Transform::Array{{left.x, alt_up.x, fwd.x, origin.x, left.y, alt_up.y, fwd.y,
                                       origin.y, left.z, alt_up.z, fwd.z, origin.z, Number(0),
  ...
  Transform Transform::multiplyFromRight(const Transform& other) const {
    ...
    for (int i = 0; i < 4; ++i) {
      for (int j = 0; j < 4; ++j) {
        Number sum = 0;
        for (int k = 0; k < 4; ++k) sum += (*this)(i, k) * other(k, j);
        result(i, j) = sum;
      }
    }
  ...
  inline static int _parseScalars(const std::string& str, T* numbers, int amount, Func func) {
    ...
    for (counter = 0; counter < amount && offset < str.size(); ++counter) {
      try {
        ...
        offset += off;
        if (offset >= str.size()) return counter + 1;
        if (std::ispunct(str[offset])) offset += 1;
      } catch (...) {
        return counter;
      }
    }
  ...
  inline static Integer _parseInteger(const std::string& str, Integer* numbers, int amount) {
    return _parseScalars(str, numbers, amount, [](const std::string& s, size_t* idx) {
      return (Integer)std::stoll(s, idx);
    });
  }
  ...
  static std::string unpackValues(const char* str, const ArgumentContainer& cnt) {
    ...
    for (int i = 0; str[i];) {
      if (str[i] == '$') {
        ...
        if (!variable.empty()) {
          if (!cnt.count(variable)) throw std::runtime_error("Unknown variable " + variable);
...
#define OT_PF(x) (1 << (x))
  ...
  enum ParseFlags {
    ...
    PF_C_SCENE = PF_C_OBJECTGROUP | PF_ALIAS | PF_SENSOR | PF_TEXTURE | PF_BSDF | PF_SUBSURFACE
                 | PF_INTEGRATOR | PF_EMITTER | PF_SHAPE | PF_MEDIUM | PF_PHASE | PF_INCLUDE
                 | PF_NULL,
    PF_C_SENSOR = PF_C_OBJECTGROUP | PF_SENSOR | PF_FILM | PF_MEDIUM | PF_SAMPLER | PF_REFERENCE,
    PF_C_SHAPE = PF_C_OBJECTGROUP | PF_BSDF | PF_SUBSURFACE | PF_SENSOR | PF_EMITTER | PF_SHAPE
                 | PF_MEDIUM | PF_TEXTURE | PF_RFILTER | PF_REFERENCE,
  ...
  static inline Property parseInteger(const ParseContext& ctx,
                                      const tinyxml2::XMLElement* element) {
    Integer value;
    return unpackInteger(element->Attribute("value"), ctx.Arguments, &value)
               ? Property::fromInteger(value)
               : Property();
  }

  static inline Property parseFloat(const ParseContext& ctx, const tinyxml2::XMLElement* element) {
    Number value;
    return unpackNumber(element->Attribute("value"), ctx.Arguments, &value)
               ? Property::fromNumber(value)
               : Property();
  }
  ...
  static inline Property parseVector(const ParseContext& ctx, const tinyxml2::XMLElement* element) {
    ...
    if (!attrib) {  // Try legacy way
      Number x, y, z;
      if (!unpackNumber(element->Attribute("x"), ctx.Arguments, &x)) return Property();
      if (!unpackNumber(element->Attribute("y"), ctx.Arguments, &y)) return Property();
      if (!unpackNumber(element->Attribute("z"), ctx.Arguments, &z)) return Property();

      return Property::fromVector(Vector(x, y, z));
    } else {
      Vector v;
      if (!unpackVector(attrib, ctx.Arguments, &v)) return Property();
  ...
  static Property parseRGB(const ParseContext& ctx, const tinyxml2::XMLElement* element) {
    ...
    if (!attrib) {  // Try legacy way
      Number r, g, b;
      if (!unpackNumber(element->Attribute("r"), ctx.Arguments, &r)) return Property();
      if (!unpackNumber(element->Attribute("g"), ctx.Arguments, &g)) return Property();
      if (!unpackNumber(element->Attribute("b"), ctx.Arguments, &b)) return Property();
  ...
  static Property parseBlackbody(const ParseContext& ctx, const tinyxml2::XMLElement* element) {
    Number temp, scale;
    if (!unpackNumber(element->Attribute("temperature"), ctx.Arguments, &temp)) return Property();

    if (!unpackNumber(element->Attribute("scale"), ctx.Arguments, &scale)) scale = Number(1);
  ...
  Transform parseTransformTranslate(const ParseContext& ctx, const tinyxml2::XMLElement* element) {
    ...
    auto value = element->Attribute("value");
    if (value) {
      if (!unpackVector(value, ctx.Arguments, &delta)) delta = Vector(0, 0, 0);
    } else {
      if (!unpackNumber(element->Attribute("x"), ctx.Arguments, &delta.x)) delta.x = 0;
      if (!unpackNumber(element->Attribute("y"), ctx.Arguments, &delta.y)) delta.y = 0;
      if (!unpackNumber(element->Attribute("z"), ctx.Arguments, &delta.z)) delta.z = 0;
    }
  ...
  Transform parseTransformScale(const ParseContext& ctx, const tinyxml2::XMLElement* element) {
    ...
    if (uniformScaleA) {
      if (!unpackVector(uniformScaleA, ctx.Arguments, &scale, Number(1))) scale = Vector(1, 1, 1);
    } else {
      if (!unpackNumber(element->Attribute("x"), ctx.Arguments, &scale.x)) scale.x = 1;
      if (!unpackNumber(element->Attribute("y"), ctx.Arguments, &scale.y)) scale.y = 1;
      if (!unpackNumber(element->Attribute("z"), ctx.Arguments, &scale.z)) scale.z = 1;
    }
  ...
  Transform parseTransformRotate(const ParseContext& ctx, const tinyxml2::XMLElement* element) {
    ...
    if (value) {
      if (!unpackVector(value, ctx.Arguments, &axis)) axis = Vector(0, 0, 1);
    } else {
      axis = Vector(0, 0, 0);
      bool hasData = false;
      if (unpackNumber(element->Attribute("x"), ctx.Arguments, &axis.x)) hasData = true;
      if (unpackNumber(element->Attribute("y"), ctx.Arguments, &axis.y)) hasData = true;
      if (unpackNumber(element->Attribute("z"), ctx.Arguments, &axis.z)) hasData = true;

      if (!hasData) axis = Vector(0, 0, 1);
    }
  ...
  Transform parseTransformLookAt(const ParseContext& ctx, const tinyxml2::XMLElement* element) {
    ...
    if (!unpackVector(element->Attribute("up"), ctx.Arguments, &up)) up = Vector(0, 0, 1);
  ...
  Transform parseTransformMatrix(const ParseContext& ctx, const tinyxml2::XMLElement* element) {
    auto value = element->Attribute("value");
    if (!value) return Transform::fromIdentity();
    ...
    // Row-Major
    if (c == 9) {
      return Transform(Transform::Array{{tmp[0], tmp[1], tmp[2], Number(0), tmp[3], tmp[4], tmp[5],
                                         Number(0), tmp[6], tmp[7], tmp[8], Number(0), Number(0),
                                         Number(0), Number(0), Number(1)}});
    }
#ifndef TPM_NO_EXTENSIONS
    else if (c == 12) {  // Official mitsuba does not support this
      return Transform(
          Transform::Array{{tmp[0], tmp[1], tmp[2], tmp[3], tmp[4], tmp[5], tmp[6], tmp[7], tmp[8],
                            tmp[9], tmp[10], tmp[11], Number(0), Number(0), Number(0), Number(1)}});
    }
#endif
    else if (c == 16) {
      return Transform(
  ...
  } _transformParseElements[] = {{"translate", parseTransformTranslate},
  ...
  Transform parseInnerMatrix(const ParseContext& ctx, const tinyxml2::XMLElement* element) {
    ...
    for (auto childElement = element->FirstChildElement(); childElement;
  ...
  } _propertyParseElement[] = {{"integer", parseInteger},
                               {"float", parseFloat},
                               {"vector", parseVector},
                               {"point", parseVector},
                               {"boolean", parseBool},
  ...
  bool parseParameter(Object* obj, const ParseContext& ctx, const tinyxml2::XMLElement* element) {
    ...
    for (int i = 0; _propertyParseElement[i].Name; ++i) {
      if (strcmp(element->Name(), _propertyParseElement[i].Name) == 0) {
        ...
        if (prop.isValid()) obj->setProperty(lcname, prop);
        return true;
      }
    }
  ...
  static void handleAlias(IDContainer& idcontainer, const tinyxml2::XMLElement* element) {
    ...
    if (!id || !as) throw std::runtime_error("Invalid alias element");

    if (!idcontainer.hasID(id)) throw std::runtime_error("Unknown id " + std::string(id));
  ...
  static void handleDefault(ArgumentContainer& cnt, const tinyxml2::XMLElement* element) {
    ...
    if (!name || !value) throw std::runtime_error("Invalid default element");

    if (!cnt.count(name)) cnt[name] = value;
  }

  static void handleReference(Object* obj, const ParseContext& ctx, const IDContainer& ids,
                              const tinyxml2::XMLElement* element, int flags) {
    auto id = element->Attribute("id");
    auto name = element->Attribute("name");

    if (!id) throw std::runtime_error("Invalid ref element");

    const auto ref_id = unpackValues(id, ctx.Arguments);

    if (!ids.hasID(ref_id)) throw std::runtime_error("Id " + ref_id + " does not exists");
  ...
  static void parseObject(Object*, const ParseContext&, IDContainer&, const tinyxml2::XMLElement*,
                          int);
  static void handleInclude(Object* obj, const ParseContext& ctx, IDContainer& ids,
                            const tinyxml2::XMLElement* element) {
    // TODO: Any default statement inside a include is not visible in the parent scope. But should
    // that not be the case?

    auto filename = element->Attribute("filename");
    if (!filename) throw std::runtime_error("Invalid include element");
  ...
  static const struct {
    ...
    int Flags;
      ...
      = {{"bsdf", OT_BSDF, PF_C_BSDF},          {"emitter", OT_EMITTER, PF_C_EMITTER},
         {"film", OT_FILM, PF_C_FILM},          {"integrator", OT_INTEGRATOR, PF_C_INTEGRATOR},
         {"medium", OT_MEDIUM, PF_C_MEDIUM},    {"phase", OT_PHASE, PF_C_PHASE},
         {"sampler", OT_SAMPLER, PF_C_SAMPLER}, {"sensor", OT_SENSOR, PF_C_SENSOR},
         {"shape", OT_SHAPE, PF_C_SHAPE},       {"subsurface", OT_SUBSURFACE, PF_C_SUBSURFACE},
         {"rfilter", OT_RFILTER, PF_C_RFILTER}, {"texture", OT_TEXTURE, PF_C_TEXTURE},
         {"volume", OT_VOLUME, PF_C_VOLUME},    {nullptr, ObjectType(0), 0}};

  static void parseObject(Object* obj, const ParseContext& ctx, IDContainer& ids,
                          const tinyxml2::XMLElement* element, int flags) {
    ...
    for (auto childElement = element->FirstChildElement(); childElement;
         childElement = childElement->NextSiblingElement()) {
      if ((flags & PF_PARAMETER) && parseParameter(obj, nextCtx, childElement)) continue;
  ...
  public:
    static Scene loadFromXML(const SceneLoader& loader, const tinyxml2::XMLDocument& xml) {
      if (xml.Error()) throw std::runtime_error(xml.ErrorStr());

      const auto rootScene = xml.RootElement();
      if (!rootScene) throw std::runtime_error("Root element is null");
      ...
      try {
        _parseVersion(rootScene->Attribute("version"), scene.mVersionMajor, scene.mVersionMinor,
                      scene.mVersionPatch);
      } catch (...) {
        throw std::runtime_error("Invalid version element");
      }

      const bool convertFromCamelCase
          = !loader.mDisableLowerCaseConversion && (scene.mVersionMajor == 0);
      parseObject(&scene,
                  ParseContext{loader.mArguments, loader.mLookupPaths, convertFromCamelCase},
                  idcontainer, rootScene, PF_C_SCENE);
